defmodule Xairo.Image.Pdf do
  ...
  As an example, the following code
      ...
      iex> pdf = Pdf.new(100, 100)
      iex> pdf |> Xairo.move_to({10, 10}) |> Xairo.line_to({90, 90})

  will produce a 100x100 pixel PDF image with a line from (10, 10) to (90, 90).
  ...
  @doc """
    ...
    ## Example
      ...
      iex> Pdf.new("test.pdf", 100, 100)

    creates a new image "test.pdf" that is 100x100 pixels in size, and

      iex> Pdf.new("test.pdf", 100, 100, scale: 3)

    creates a new image "test.pdf" that is 300x300 pixels in size.
  ...
  def new(filename, width, height, opts \\ []) do
    ...
    {:ok, resource} = Xairo.Native.new_pdf_image(scaled_width, scaled_height, filename)
